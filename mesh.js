/**
 * @fileoverview mesh - 3D surface mesh
 * @author Anthony Li
 */
 ...
 */
    ...
    computeAABB(){
        ...
        for(var i = 0; i < this.numVertices; i++){
            ...
            for (var k = 0; k < v.length; k++){
                if (v[k] > this.maxXYZ[k]){
                    this.maxXYZ[k] = v[k];
                }
                if (v[k] < this.minXYZ[k]){
                    this.minXYZ[k] = v[k];
                }
    ...
    */
    getAABB(minXYZ,maxXYZ){
        minXYZ = this.minXYZ.slice();
        maxXYZ = this.maxXYZ.slice();
    }
    ...
    loadFromOBJ(fileText)
    {
        ...
        for(var i = 0;i < lines.length;i++){
            ...
            if (currentLine[0] == '#'){
                ...
                var values = currentLine.split(/\b\s+(?!$)/);;
                this.vBuffer.push(parseFloat(values[1]));
                this.vBuffer.push(parseFloat(values[2])-1.8);
